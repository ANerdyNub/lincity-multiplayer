...
/* Include header for terminals too */
#if !defined (LC_X11) && !defined (WIN32) && !defined (SVGALIB)
...
//    const char default_dir[] = "D:\\LINCITY";	/* For GCS's use */
    ...
    /* Check 2: directory containing the executable */
    if (GetModuleFileName (NULL, exe_path, LC_PATH_MAX) > 0) {
	p = strrchr (exe_path, '\\');
	if (p) {
	    *p = '\0';
	    snprintf (LIBDIR, LC_PATH_MAX, "%s\\%s", exe_path, searchfile);
	    if (_access (LIBDIR, 0) != -1) {
		*p = '\0';
		strncpy (LIBDIR, exe_path, LC_PATH_MAX);
		return;
	    }
	}
    }
    ...
    if (_access (searchfile, 0) != -1) {
	GetCurrentDirectory (LC_PATH_MAX, LIBDIR);
	return;
    }
...
load_lincityrc (void)
{
	...
	if (sscanf(buf,"no_autosave=%d",&arg)==1) {
	    no_autosave = !!arg;
	    continue;
	}
    }
...
save_lincityrc (void)
{
    ...
    fprintf (fp,
	"# Set this if you don't want the game to automatically save\n"
	"# the game once per year.\n"
	"no_autosave=%d\n\n",
	no_autosave
	);
