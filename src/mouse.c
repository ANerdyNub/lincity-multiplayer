...
static int mb_draw (int cxp, int cyp, int flag);
...
cs_mouse_handler (int enc_button, int dx, int dy)
{
  ...
  /* should dismiss cb? */
	...
	case LC_MOUSE_LEFTBUTTON:
	    ...
	    if (mb_flag)
		mb_draw(cs_mouse_x, cs_mouse_y, MT_SUCCESS);
	    break;
	case LC_MOUSE_RIGHTBUTTON:
	    break;
	case LC_MOUSE_MIDDLEBUTTON:
	...
	if (mb_flag)
	    mb_draw(cs_mouse_x, cs_mouse_y, MT_CONTINUE);
    }
...
do_mouse_main_win (int px, int py, int button)
{
	...
	/* Client: skip expensive place_item (incl. map_power_grid).
	   Let host validate and broadcast back. */
	network_client_send_action (network_socket, MSG_BUILD,
				    x, y, selected_module_type);
    } else
#endif
...
#if defined (NETWORK_ENABLE)
	if (network_game && network_is_host)
	    network_host_broadcast_action_all (MSG_BUILD,
					   x, y, selected_module_type);
#endif
	break;
    case -1:
...
#if defined (NETWORK_ENABLE)
    if (network_game && !network_is_host)
	network_client_send_action (network_socket, MSG_BULLDOZE,
				    xx, yy, 0);
    else if (network_game && network_is_host)
	network_host_broadcast_action_all (MSG_BULLDOZE,
				       xx, yy, 0);
#endif
}
...
#if defined (NETWORK_ENABLE)
    if (network_game && MP_INFO(mcbx,mcby).flags != old_flags) {
	if (network_is_host)
	    network_host_broadcast_action_all (MSG_MFLAGS, mcbx,
					   mcby, MP_INFO(mcbx,mcby).flags);
	else
	    network_client_send_action (network_socket, MSG_MFLAGS, mcbx, mcby,
					MP_INFO(mcbx,mcby).flags);
    }
#endif
}
...
#define STATUS_MESSAGE_LENGTH 80
...
#if defined (NETWORK_ENABLE)
	if (network_game && !network_is_host)
	    network_client_send_action (network_socket, MSG_DRAG_END, 0, 0, 0);
#endif
	...
	if (!draw_ret) {
	    ...
	    status_message(0,0);
#if defined (NETWORK_ENABLE)
	    /* Client: preview flags (FLAG_MULTI_TRANSPORT) may have
	       been overwritten by network recv between drag events.
	       Still try to place — host validates. */
	    if (network_game && !network_is_host) {
		if ((draw_ret = do_mt_draw(ox, cx, oy, cy, mt_perm))) {
		    print_total_money ();
		}
	    }
#endif
...
#if defined (NETWORK_ENABLE)
	if (network_game && !network_is_host)
	    network_client_send_action (network_socket, MSG_DRAG_START, 0, 0, 0);
#endif
 ...
 * Multi-bulldozing
 * ---------------------------------------------------------------------- */
static int mb_x1, mb_y1, mb_x2, mb_y2;

static int
mb_draw (int cxp, int cyp, int flag)
{
    static int ox, oy;
    int cx, cy;

    if (flag != MT_START && !mb_flag)
	return 0;
	...
	{
	    int x, y, t, mx, my;
	    int rx1 = mb_x1, ry1 = mb_y1, rx2 = mb_x2, ry2 = mb_y2;

	    if (rx1 > rx2) { t = rx1; rx1 = rx2; rx2 = t; }
	    if (ry1 > ry2) { t = ry1; ry1 = ry2; ry2 = t; }
	    if (rx1 < 0) rx1 = 0;
	    if (ry1 < 0) ry1 = 0;
	    if (rx2 >= WORLD_SIDE_LEN) rx2 = WORLD_SIDE_LEN - 1;
	    if (ry2 >= WORLD_SIDE_LEN) ry2 = WORLD_SIDE_LEN - 1;
		...
		for (x = rx1; x <= rx2; x++)
		{
		    if (MP_TYPE(x,y) == CST_USED)
		    {
			mx = MP_INFO(x,y).int_1;
			my = MP_INFO(x,y).int_2;
		    }
		    else
		    {
			mx = x;
    ...
    {
        int rx1 = mb_x1, ry1 = mb_y1, rx2 = mb_x2, ry2 = mb_y2;
        int t, px1, py1, px2, py2;

        if (rx1 > rx2) { t = rx1; rx1 = rx2; rx2 = t; }
        if (ry1 > ry2) { t = ry1; ry1 = ry2; ry2 = t; }

        px1 = scr.main_win.x + (rx1 - main_screen_originx) * 16;
        py1 = scr.main_win.y + (ry1 - main_screen_originy) * 16;
        px2 = scr.main_win.x + (rx2 + 1 - main_screen_originx) * 16 - 1;
        py2 = scr.main_win.y + (ry2 + 1 - main_screen_originy) * 16 - 1;

        Fgl_fillbox(px1, py1, px2 - px1 + 1, py2 - py1 + 1, yellow(8));
        Fgl_hline(px1, py1, px2, yellow(31));
        Fgl_hline(px1, py2, px2, yellow(31));
        Fgl_line(px1, py1, px1, py2, yellow(31));
        Fgl_line(px2, py1, px2, py2, yellow(31));
    }
